...
export class ItinerairesService{
    ...
    itinerairesLegerDeTransporter(idTransporter:number){
        this.loadTonken();
        return this.http.get(this.adServer+":8080/itinerairesLegerTransporter/"+idTransporter
        , {headers:new HttpHeaders({'Authorization':this.jwToken})})
        .pipe(map(res => {return res}));
    }
    ...
    itinerairesLegerDeCamion(idCamion:number){
        this.loadTonken();
        return this.http.get(this.adServer+":8080/itinerairesLegerCamion/"+idCamion
        , {headers:new HttpHeaders({'Authorization':this.jwToken})})
        .pipe(map(res => {return res}));
    }
